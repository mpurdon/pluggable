...
const (
	cipherPlugin = "plugins/cipher.so"
)
...
func main() {
	...
	// Load Cipher plugin
	pluginModule, err := plugin.Open(cipherPlugin)
	if err != nil {
	...
	fmt.Printf("Encrypted with Caeser (Shifted 4): \t\t%s\n", encryptedC4)

	decryptedC4 := decryptCaesarFunc(4, encryptedC4)
	fmt.Printf("Decrypted with Caeser (Shifted 4): \t\t%s\n", decryptedC4)
